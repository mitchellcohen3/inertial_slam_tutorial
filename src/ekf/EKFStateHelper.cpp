...
void EKFStateHelper::EKFPropagation(
    std::shared_ptr<EKFState> state,
  ...
  if (order_NEW.empty() || order_OLD.empty()) {
    // PRINT_ERROR(RED "EKFStateHelper::EKFPropagation() - Called with empty
    // variable arrays!\n" RESET);
    LOG(ERROR) << "EKFEKFStateHelper::EKFPropagation() - Called with empty "
                  "variable arrays!\n";
  ...
  for (size_t i = 0; i < order_NEW.size() - 1; i++) {
    if (order_NEW.at(i)->id() + order_NEW.at(i)->size() !=
        order_NEW.at(i + 1)->id()) {
      LOG(ERROR) << "EKFEKFStateHelper::EKFPropagation() - Called with "
                    "non-contiguous state elements!\n";
      LOG(ERROR)
          << "EKFEKFStateHelper::EKFPropagation() - This code only support a "
             "state transition which is in the same order as the state\n";
      std::exit(EXIT_FAILURE);
    }
  ...
  for (int i = 0; i < diags.rows(); i++) {
    if (diags(i) < 0.0) {
      LOG(ERROR) << "EKFEKFStateHelper::EKFPropagation() - diagonal at " << i
                 << " is " << diags(i) << "\n";
      found_neg = true;
    }
  }
...
void EKFStateHelper::EKFUpdate(
    std::shared_ptr<EKFState> state,
    const std::vector<std::shared_ptr<Type>> &H_order, const Eigen::MatrixXd &H,
  ...
  Eigen::MatrixXd P_small =
      EKFStateHelper::get_marginal_covariance(state, H_order);
  ...
  for (int i = 0; i < diags.rows(); i++) {
    if (diags(i) < 0.0) {
      LOG(ERROR) << "EKFEKFStateHelper::EKFUpdate() - diagonal at " << i
                 << " is " << diags(i) << "\n";
      found_neg = true;
    }
  }
  ...
  // // objects NOTE: is this the best place to put this update logic???
  // probably.. if (state->_options.do_calib_camera_intrinsics) {
...
void EKFStateHelper::set_initial_covariance(
    std::shared_ptr<EKFState> state, const Eigen::MatrixXd &covariance,
...
Eigen::MatrixXd EKFStateHelper::get_marginal_covariance(
    std::shared_ptr<EKFState> state,
...
Eigen::MatrixXd
EKFStateHelper::get_full_covariance(std::shared_ptr<EKFState> state) {
...
void EKFStateHelper::marginalize(std::shared_ptr<EKFState> state,
  ...
  if (std::find(state->_variables.begin(), state->_variables.end(), marg) ==
    ...
    LOG(ERROR) << "EKFEKFStateHelper::marginalize() - Called to marginalize a "
                  "variable that is not in the state!\n";
    std::exit(EXIT_FAILURE);
  }
...
std::shared_ptr<Type>
EKFStateHelper::clone(std::shared_ptr<EKFState> state,
  ...
  if (new_clone == nullptr) {
    LOG(ERROR) << "EKFEKFStateHelper::clone() - Called to clone a variable "
...
bool EKFStateHelper::initialize(
    std::shared_ptr<EKFState> state, std::shared_ptr<Type> new_variable,
  ...
  EKFStateHelper::initialize_invertible(state, new_variable, H_order, Hxinit,
                                        H_finit, Rinit, resinit);

  // Update with updating portion
  if (Hup.rows() > 0) {
    EKFStateHelper::EKFUpdate(state, H_order, Hup, resup, Rup);
  }
  return true;
}

void EKFStateHelper::initialize_invertible(
    std::shared_ptr<EKFState> state, std::shared_ptr<Type> new_variable,
  ...
  if (std::find(state->_variables.begin(), state->_variables.end(),
    ...
    LOG(ERROR) << "EKFEKFStateHelper::initialize_invertible() - "
                  "Called to initialize a variable that is already in the "
                  "state!\n";
    LOG(ERROR) << "EKFEKFStateHelper::initialize_invertible() - Found this "
                  "variable at "
               << new_variable->id() << " in covariance\n";
    std::exit(EXIT_FAILURE);
  }
...
// void EKFStateHelper::augment_clone(std::shared_ptr<EKFState> state,
...
// void EKFStateHelper::marginalize_old_clone(std::shared_ptr<EKFState> state) {
...
//     EKFStateHelper::marginalize(state, state->_clones_IMU.at(marginal_time));
...
// void EKFStateHelper::marginalize_slam(std::shared_ptr<EKFState> state) {
...
//       EKFStateHelper::marginalize(state, (*it0).second);
